...
import { StartConversationFormSchema } from './start-conversation-dialog.schema';
...
/**
 * UI state hook for the "Start Conversation" dialog.
 ...
 */
export function useStartConversationDialog(onSubmit: SubmitHandler) {
  ...
  const submit = async () => {
    const parsed = StartConversationFormSchema.safeParse(values);
    if (!parsed.success) {
