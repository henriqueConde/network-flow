...
export type AddReplyDialogProps = {
  isOpen: boolean;
  values: { body: string; sender: 'user' | 'contact'; sentAt: string; contactId?: string };
  errors: Partial<Record<'body' | 'sender' | 'sentAt' | 'contactId', string>>;
  isAddingReply: boolean;
  conversation: ConversationDetail | null;
  config: typeof CONVERSATION_DETAIL_CONFIG;
  onClose: () => void;
  onChangeField: (field: 'body' | 'sender' | 'sentAt' | 'contactId', value: string | 'user' | 'contact') => void;
  onSubmit: () => void;
  // AI Assistant props
