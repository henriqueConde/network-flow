...
import {
  ...
  addContactToConversationBody,
} from '@/backend/features/conversations/http/conversations.schemas';

/**
 * POST /api/conversations/[id]/contacts
...
export async function POST(
  ...
  try {
    ...
    const body = addContactToConversationBody.parse(json);

    const result = await addContactToConversation({
      userId: user.id,
      conversationId: id,
      body,
    });
    ...
    if (!result) {
      throw new NotFoundError('Conversation or contact not found');
    }
    ...
    console.error('[api/conversations/[id]/contacts] Unexpected error:', errorMessage, errorStack || '');
